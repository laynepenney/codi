  ...
  execute: async (args: string, context: CommandContext): Promise<string | null> => {
    ...
    switch (action) {
      ...
      case 'remove': {
        if (!target) {
          if (context.selectSession) {
            ...
            const selection = await context.selectSession(sessions, 'Select a session to delete:');
            if (!selection) {
              return null;
            }
            const deleted = deleteSession(selection.name);
            if (deleted) {
              if (currentSessionName === selection.name) {
                currentSessionName = null;
                context.setSessionName?.(null);
              }
              return `__SESSION_DELETED__:${selection.name}`;
            }
            return `__SESSION_NOT_FOUND__:${selection.name}`;
          }
        ...
        let deleted = deleteSession(target);
        if (!deleted) {
          const matches = findSessions(target);
          if (matches.length === 1) {
            deleted = deleteSession(matches[0].name);
            if (deleted) {
              if (currentSessionName === matches[0].name) {
                currentSessionName = null;
                context.setSessionName?.(null);
              }
              return `__SESSION_DELETED__:${matches[0].name}`;
            }
          } else if (matches.length > 1 && context.selectSession) {
            const selection = await context.selectSession(
              matches,
            ...
            deleted = deleteSession(selection.name);
            if (deleted) {
              if (currentSessionName === selection.name) {
                currentSessionName = null;
                context.setSessionName?.(null);
      ...
      case 'show': {
        if (!target) {
          ...
          if (!currentSessionName) {
            if (context.selectSession) {
              ...
              const selection = await context.selectSession(sessions, 'Select a session to view:');
              if (!selection) {
                return null;
              }
              const session = loadSession(selection.name);
        ...
        let session = loadSession(target);
        if (!session) {
          const matches = findSessions(target);
          if (matches.length === 1) {
            session = loadSession(matches[0].name);
            ...
            session = loadSession(selection.name);
          } else if (matches.length > 1) {
            let list = `__SESSION_MULTIPLE__:${target}:`;
            for (const info of matches.slice(0, 5)) {
              list += `\n${formatSessionInfo(info)}`;
            }
            return list;
